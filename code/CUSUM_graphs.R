...
aggr8 <- subset(aggr5, season>=2006)

aggr8$Sweek[aggr8$Sweek >= 27 & aggr8$YoDi !=2010] <- aggr8$Sweek[aggr8$Sweek >= 27 & aggr8$YoDi !=2010] + 1 
aggr8$Sweek[aggr8$WoDi == 53] <- 27

# Cumulative sums

aggr8 <- aggr8[order(aggr8$season),]

aggr8$sum <- aggr8$excess
aggr8$sum[is.na(aggr8$sum)] <- 0
aggr8$sum <- cumsum(aggr8$sum)

aggr8$Zsum <- aggr8$zscore
aggr8$Zsum[is.na(aggr8$Zsum)] <- 0
aggr8$Zsum <- cumsum(aggr8$Zsum)
...
png(sprintf("%s/GRAPH-COMBINED-CUSUM-%s-%s-%s-%s.png", glb$CUMULATIVE, glb$country, glb$GROUP, glb$YOSI, glb$WOSI), 
...
mtext(sprintf("ANALYSIS CUMULATIVE SUMS\n%s-%s years-%s-%s\nCUSUM set up at 0 week %s-%s", glb$country, glb$GROUP, glb$YOSI, glb$WOSI, glb$YOSI, glb$WOSI),
