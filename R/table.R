...
tableMOMO <- function(aggr) {
  tbl <- aggr[(aggr$wk > attr(aggr, "WEEK")-attr(aggr, "delayCorr")-6 & aggr$wk <= attr(aggr, "WEEK")), c("wk", "YoDi", "WoDi", "nb2", "nbc", "Pnb", "excess", "zscore", "FLAG")]
  tbl$nbc[tbl$wk <= attr(aggr, "WEEK") - attr(aggr, "delayCorr")] <- NA
  tbl$wk <- NULL
  ...
  tbl <- cbind(unique(aggr$Version), attr(aggr, "group"), names(tbl), t(tbl))
