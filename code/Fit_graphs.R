...
fitGraphsMOMO <- function(output, dirs, group=NA) {
  ...
  lapply(output, function(x) {
    ...
    png(sprintf("%s/GRAPH-FIT-%s-%s-%s-%s.png", dirs$FIT, attr(final, "country"), attr(final, "group"), attr(final, "YOSI"), attr(final, "WOSI")),
    ...
    with(subset(final, wk <= attr(final, "WEEK")), {
    ...
    ac1 <- acf(subset(final, wk <= attr(final, "WEEK"))[,DATA], lag.max=75, plot=FALSE)
    ac1$acf[1] <- 0
    plot(ac1, ci.type="ma", main=NA, xlab=NA, ylab=NA, bty="l")
    mtext("AUTO-CORRELATION", side=2, cex=1.5, line=5, xpd=TRUE)

    ac1 <- pacf(subset(final, wk <= attr(final, "WEEK"))[,DATA], lag.max=75, plot=FALSE)
    ac1$acf[1] <- 0
    ...
    # histogram
    with(final, {
      resi <- get(RESI)[(WoDi>attr(final, "SPRING")[1] & WoDi<attr(final, "SPRING")[2]) | (WoDi>attr(final, "AUTUMN")[1] & WoDi<attr(final, "AUTUMN")[2])]
    ...
    mtext(sprintf("Analysis Model Fit\n%s-%s years-%s-%s", attr(final, "country"), attr(final, "group"), attr(final, "YOSI"), attr(final, "WOSI")),
