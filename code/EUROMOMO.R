...
EUROMOMOoutput <- function(aggr, useAUTOMN=FALSE, datesISO=TRUE) {
  ...
  aggr2$source <- attr(aggr, "source")
  aggr2$country <- attr(aggr, "country")
  ...
  aggr2$DateoA <- attr(aggr, "DoA")
  aggr2$YoAi <- attr(aggr, "YOAI")
  aggr2$WoAi <- attr(aggr, "WOAI")

  aggr2$flag[aggr2$wk>attr(aggr, "WEEK2") - 7] <- 1
  aggr2$flag[aggr2$wk<attr(aggr, "WEEK2") - 6] <- 0
