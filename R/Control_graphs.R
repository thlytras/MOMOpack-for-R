...
controlGraphsMOMO <- function(output, dirs, group=NA) {
  ...
  lapply(output, function(x) {
    ...
    a$COND1[a$YoDi >= attr(a, "YOSI")-2 & a$YoDi <= attr(a, "YOSI")] <- 1
    a$COND2[a$YoDi >= attr(a, "YOSI")-3 & a$YoDi <= attr(a, "YOSI")] <- 1
	...
	abline(v=c(attr(a, "WEEK"), attr(a, "WEEK2")), col="red", lwd=2)
	abline(v=c(attr(a, "PRWEEK")), col="green", lwd=2)
    ...
    png(sprintf("%s/GRAPH-CONTROL-%s-%s-%s-%s.png", dirs$CONTROL, attr(a, "group"), attr(a, "country"), attr(a, "YOSI"), attr(a, "WOSI")),
	width=2000*2, height=1200*3, pointsize=21, res=144)
    layout(matrix(1:6,ncol=2,byrow=TRUE))

    plotControlGraph(data=subset(a, COND1==1), cex=0.67,
	y=c("nb", paste("WR", 0:attr(a, "delayCorr"), sep=""), "closed"),
	line=TRUE, point=FALSE, lwd=c(4, rep(2, attr(a, "delayCorr")+1)), lty=1,
	col = c("steelblue4", topo.colors(attr(a, "delayCorr")+1), "lavenderblush3"),
	main = c(sprintf("CONTROL FLOW OF INFORMATION %s", VERSION),
	    sprintf("Number of Deaths - %s - Age group %s years - week %s-%s", 
	      attr(a, "country"), attr(a, "group"), attr(a, "WOSI"), attr(a, "YOSI")),
	    sprintf("Aggregation the %s, correction for the %s previous weeks", 
	      format(attr(a, "DoA"), format="%d - %m - %Y"), attr(a, "delayCorr")),
	...
	legend = c("Number of deaths known in the series", 
	    paste("Registered up to", 0:attr(a, "delayCorr"), "weeks after the week of death"),
    ...
    for (i in 0:3) { # 3 is hardcoded in the original file (Control_graphs.do). Equals to attr(a, "delayCorr")
      plotControlGraph(data=subset(a, COND2==1), cex=0.67,
	  ...
	  main = c(sprintf("CONTROL QUALITY OF THE PREDICTION AT WEEK %s %s", c("OF STUDY", 1:3)[i+1], VERSION),
		...
		attr(a, "country"), attr(a, "group")),
	      sprintf("Aggregation on %s for week %s-%s", 
		format(attr(a, "DoA"), format="%d - %m - %Y"), attr(a, "WOSI"), attr(a, "YOSI")),
